...
int <- ggplot(anova_results_int_tuk[[2]], aes(x=Group, y=mean)) +
  ...
  facet_wrap(~factor(Lipid, levels=unique(anova_results_int_tuk[[1]]$Lipid)), scales = "free", ncol = 5) +
  ylab("log2 lipid concentration (nmol/g)")+
  geom_text(x = Inf, y = -Inf, 
...
write.xlsx(as.data.frame(anova_results_class %>% 
                           left_join(lipidomics_data %>% select(Lipid, Type)) %>% 
                           mutate(Type = str_remove(Type, fixed("(lipid class)"))) %>% 
                           relocate(Type, .after = Lipid)), file = "Supplementary table 1.xlsx", sheetName = "Suppl table 1F", 
  col.names = TRUE, row.names = FALSE, append = T)

# anova results (ratios significant by group)
write.xlsx(as.data.frame(anova_results_class_group%>% 
                           left_join(lipidomics_data %>% select(Lipid, Type)) %>% 
                           mutate(Type = str_remove(Type, fixed("(lipid class)"))) %>% 
                           relocate(Type, .after = Lipid)), file = "Supplementary table 1.xlsx", sheetName = "Suppl table 1G", 
  col.names = TRUE, row.names = FALSE, append = T)
